...
build:
	@echo " -> Building"
	mkdir -p bin
	CGO_ENABLED=0 go build -o bin/terraform-provider-ssh
	@echo "Built terraform-provider-ssh"

acctest: build
	# to run only certain tests, run something of the form:  make acctest TESTARGS='-run=TestAccSsh_DiskSlot'
	TF_ACC=1 go test ./ssh $(TESTARGS)

install: build
	cp bin/terraform-provider-ssh $$GOPATH/bin/terraform-provider-ssh

local-dev-install: build
	@echo "$(CURRENT_VERSION_MICRO)"
	@echo "$(KERNEL)"
	@echo "$(ARCH)"
	mkdir -p ~/.terraform.d/plugins/localhost/loafoe/ssh/$(CURRENT_VERSION_MICRO)/$(KERNEL)_$(ARCH)/
	cp bin/terraform-provider-ssh ~/.terraform.d/plugins/localhost/loafoe/ssh/$(CURRENT_VERSION_MICRO)/$(KERNEL)_$(ARCH)/
